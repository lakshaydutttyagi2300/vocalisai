...
// Bulk enable/disable, scoped to a required category + difficulty pair -
// deliberately narrower than the single-question PATCH route, since a
// mistaken bulk action here can flip hundreds of rows at once. Built for
// reviewing large imported batches (e.g. the question-bank-full.json
// import, which lands everything isActive:false pending review) a slice
// at a time, not as a general-purpose filter tool.
export async function PATCH(req: Request) {
  ...
  const result = await db.practiceQuestion.updateMany({
    where: { category, difficulty, isActive: !isActive },
    data: { isActive },
  });
  ...
  if (result.count > 0) {
    await logAdminAction({
      ...
      after: { category, difficulty, isActive, count: result.count },
    });
  }
